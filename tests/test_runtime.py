...
from superteam_codex.runtime.g5 import apply_g5_hook_trace_signal, run_g5_hook_trace_until_stage_gate
from superteam_codex.runtime.g6 import apply_g6_hook_trace_signal, run_g6_hook_trace_until_stage_gate
from superteam_codex.runtime.g7 import (
    apply_g7_hook_trace_signal,
    inspector_report_path,
    run_g7_hook_trace_until_stage_gate,
)
from superteam_codex.runtime.stages import reset_workspace, start_run
from superteam_codex.runtime.state import StateError, load_mode, save_mode, set_lifecycle, set_stage
...
class RuntimeTests(unittest.TestCase):
    ...
    def complete_g2_inspector(self, ws: Workspace, note: str = "inspector pass") -> dict:
        ...
        apply_g2_hook_trace_signal(
            ...
            agent="inspector",
            agent_id=f"inspector-{event_id.lower().replace('.', '-')}",
        )
    ...
    def complete_g2_design_step_with_inspector(self, ws: Workspace, note: str) -> dict:
        apply_g2_hook_trace_signal(
            ...
            agent="designer",
            agent_id="agent-g2-designer-pencil",
        )
    ...
    def test_g1_events_advance_one_by_one_and_approve(self) -> None:
        ...
        approval = approve_g1(ws, note="用户批准 G1")
        ...
        self.assertIn("G1.Q4 项目是否需要 UI 界面？如果需要，采用什么 UI 工具？", text)
    ...
    def test_g1_hook_trace_exposes_spawn_and_inspector(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertEqual(result["orchestrator"]["agent_calls"][-1]["agent_id"], "agent-g1-summary")
            ...
            result = apply_g1_hook_trace_signal(
                ...
                "spawned inspector",
                agent="inspector",
                agent_id="agent-g1-inspector-summary",
            )
            ...
            result = apply_g1_hook_trace_signal(
                ...
                agent="inspector",
                agent_id="agent-g1-inspector-approval",
            )
    ...
    def test_g2_ui_plan_uses_designer_agent_result_when_provided(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            apply_g2_hook_trace_signal(
                ...
                "spawned designer",
                agent="designer",
                agent_id="agent-designer-plan",
            )
            result = apply_g2_hook_trace_signal(
                ws,
                "agent-result",
                'UI_PLAN_JSON: ["S1-登录页", "S2-员工花名册"]',
                agent="designer",
            )

            self.assertEqual(result["ui_plan"], ["S1-登录页", "S2-员工花名册"])
    ...
    def test_g2_hook_trace_flow_runs_auto_nodes_and_stops_at_user_gates(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertEqual(result["orchestrator"]["spawn_status"], "waiting_result")
            self.assertEqual(result["orchestrator"]["agent_calls"][-1]["agent_id"], "agent-g2-ui-plan")
            ...
            result = apply_g2_hook_trace_signal(
                ...
                agent="inspector",
                agent_id="agent-g2-inspector-ui-plan",
            )
            ...
            result = apply_g2_hook_trace_signal(ws, "approve-plan", "通过")
            self.assertEqual(result["active_event"], "G2.APPROVE_UI_DESIGN_PLAN")
            ...
            result = apply_g2_hook_trace_signal(
                ...
                agent="inspector",
                agent_id="agent-g2-inspector-approve-plan",
            )
            ...
            result = apply_g2_hook_trace_signal(
                ...
                agent="inspector",
                agent_id="agent-g2-inspector-design-steps",
            )
    ...
    def test_g3_hook_trace_maps_pencil_frames_to_code_plan(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
                ...
                json.dumps(
                    {
                        ...
                        "version": "2.11",
                        "children": [
                            {"id": "s1_login", "type": "frame", "name": "S1-登录页", "children": []},
                            {"id": "s2_roster", "type": "frame", "name": "S2-员工花名册", "children": []},
                        ],
                    }
                ),
                encoding="utf-8",
            ...
            self.assertEqual(result["active_event"], "G3.MAP_PENCIL_TO_CODE_TARGETS")
            self.assertIn("G3.MAP_PENCIL_TO_CODE_TARGETS.spawn_required", result["trace_hooks"])

            result = self.complete_g3_spawned_event(
                ws,
                "designer",
                "agent-g3-ui-map",
                "PASS: Pencil frames mapped to code targets",
            )
            self.assertEqual(result["active_event"], "G3.DRAFT_EXECUTION_PLAN")
            self.assertIn("G3.EXTRACT_LAYOUT_SPEC.record", result["trace_hooks"])
            self.assertIn("G3.EXTRACT_DESIGN_TOKENS.record", result["trace_hooks"])
            self.assertIn("G3.MAP_INTERACTION_STATES.record", result["trace_hooks"])
            self.assertIn("G3.WRITE_VISUAL_ACCEPTANCE.record", result["trace_hooks"])
            self.assertIn("G3.CHECK_UI_IMPLEMENTATION_CONTRACT.record", result["trace_hooks"])
            ...
            execution_artifact.write_text(
                self.executor_artifact_text("agent-g4-executor", implementation_plan),
                encoding="utf-8",
            )
            result = apply_g4_hook_trace_signal(
                ws,
            ...
            result = apply_g4_hook_trace_signal(
                ...
                "spawn inspector",
                agent="inspector",
                agent_id="agent-g4-inspector-readiness",
            )
            ...
            review_artifact.write_text(
                self.blocked_review_artifact_text("agent-g5-reviewer", load_mode(ws)["active_task_slug"]),
                encoding="utf-8",
            )
            result = apply_g5_hook_trace_signal(
                ws,
            ...
            result = apply_g4_hook_trace_signal(
                ...
                agent="executor",
                agent_id="agent-g4-executor-g5-block-repair",
            )
            self.assertIn("G4.SPAWN_EXECUTOR.spawn_record", result["trace_hooks"])
            self.complete_g4_tdd_cycle(ws, implementation_plan)
            execution_artifact.write_text(
                self.executor_artifact_text("agent-g4-executor-g5-block-repair", implementation_plan),
                encoding="utf-8",
            )
            result = apply_g4_hook_trace_signal(
                ws,
            ...
            apply_g4_hook_trace_signal(
                ...
                agent="inspector",
                agent_id="agent-g4-inspector-g5-block-repair",
            )
            ...
            apply_g5_hook_trace_signal(
                ...
                agent="reviewer",
                agent_id="agent-g5-reviewer-after-block",
            )
            review_artifact.write_text(
                self.review_artifact_text("agent-g5-reviewer-after-block", load_mode(ws)["active_task_slug"]),
                encoding="utf-8",
            )
            result = apply_g5_hook_trace_signal(
                ws,
            ...
            apply_g5_hook_trace_signal(
                ...
                agent="designer",
                agent_id="agent-g5-designer-after-block",
            )
            result = apply_g5_hook_trace_signal(
                ws,
                "agent-result",
            ...
            apply_g6_hook_trace_signal(
                ...
                agent="verifier",
                agent_id="agent-g6-verifier-fail",
            )
            verification_artifact = run_dir / "07-verification.md"
            verification_artifact.write_text(
                self.failed_verification_artifact_text("agent-g6-verifier-fail", load_mode(ws)["active_task_slug"]),
                encoding="utf-8",
            )
            ...
            result = apply_g4_hook_trace_signal(
                ...
                agent="executor",
                agent_id="agent-g4-executor-repair",
            )
            self.assertIn("G4.SPAWN_EXECUTOR.spawn_record", result["trace_hooks"])
            self.complete_g4_tdd_cycle(ws, implementation_plan)
            execution_artifact.write_text(
                self.executor_artifact_text("agent-g4-executor-repair", implementation_plan),
                encoding="utf-8",
            )
            result = apply_g4_hook_trace_signal(
                ws,
            ...
            apply_g4_hook_trace_signal(
                ...
                agent="inspector",
                agent_id="agent-g4-inspector-repair",
            )
            ...
            apply_g5_hook_trace_signal(
                ...
                agent="reviewer",
                agent_id="agent-g5-reviewer-repair",
            )
            review_artifact.write_text(
                self.review_artifact_text("agent-g5-reviewer-repair", load_mode(ws)["active_task_slug"]),
                encoding="utf-8",
            )
            result = apply_g5_hook_trace_signal(
                ws,
            ...
            apply_g5_hook_trace_signal(
                ...
                agent="designer",
                agent_id="agent-g5-designer-ui-repair",
            )
            result = apply_g5_hook_trace_signal(
                ws,
                "agent-result",
            ...
            verification_artifact = run_dir / "07-verification.md"
            verification_artifact.write_text(
                self.verification_artifact_text("agent-g6-verifier", load_mode(ws)["active_task_slug"]),
                encoding="utf-8",
            )
            result = apply_g6_hook_trace_signal(
                ws,
            ...
            inspector_report.write_text(
                self.inspector_report_text("agent-g7-inspector", load_mode(ws)["active_task_slug"]),
                encoding="utf-8",
            )
            result = apply_g7_hook_trace_signal(
                ws,
            ...
            (run_dir / "08-finish.md").write_text(
                self.finish_artifact_text("agent-g7-writer", load_mode(ws)["active_task_slug"]),
                encoding="utf-8",
            )
